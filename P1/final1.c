...
void main(){
	...
	int childReady = 0;
	int receivedReady = 0;
	int randomArray1 = rand()%4;
	int randomArray2 = rand()%4;
		...
		if(fork() == 0){
			...
			if(iCount == 5){
					...
					while(receivedReady == READY){
						int iTemp = 0;
						while(cmdCode == REQUEST){
							//printf("command is request\n");
							// iTemp++;
							// if(iTemp == 40){
								// break;
							// }
							...
							close(ptc[5][1]);
							read(ptc[5][0], &randomArrayChild2, sizeof(randomArrayChild2));
							pivotVal = numbers[randomArrayChild1][randomArrayChild2];
							...
							if(pivotVal < 0){
								cmdCode = NEW;
								//printf("===========BAD PIVOT VALUE %d DETECTED BY CHILD==============\n",pivotVal);
								break;
							}
							...
							for(int i = 0; i < 5;i++){
								...
								int temp = 0;
								for(int j = 0; j < 5; j++){
									if(pivotVal <= numbers[i][j] && 0 < pivotVal){
										temp++;
									}
									countLarger[i]=temp;
								}
						...
						//printf("received command %d from parent\n", cmdCode);
						if(cmdCode == SMALL){
							//printf("command was small\n");
							for(int i = 0; i < randomArrayChild1; i++){
								if(i < randomArrayChild1){
									for(int j = 0; j < 5; j++)
									{
										numbers[i][j] = -1;
									}
									...
									for(int j = 0; j < randomArrayChild2; j++)
									{
										numbers[i][j] = -1;
									}
								}
						...
						if(cmdCode == LARGE){
						//printf("command was large\n");
							for(int i = randomArrayChild1; i < 5; i++){
								if(i == randomArrayChild1){
									for(int j = randomArrayChild2; j < 5; j++){
										numbers[i][j] = -1;
									}
								}
								else{
	...
	close(ptc[5][0]);
	write(ptc[5][1], &randomArray2, sizeof(randomArray2));
	...
	printf("Median, %d, has been found!\n", pivotVal);
